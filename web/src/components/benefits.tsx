...
const FeatureCard = ({ title, description, icon }: FeatureCardProps) => {
  return (
    <div className="rounded-lg border border-input bg-secondary p-6 transition-all duration-300 hover:border-accent/75 hover:shadow-accent hover:shadow-lg hover:-translate-y-1">
      <div className="flex h-full flex-col">
        <div className="mb-4">{icon}</div>
        <h3 className="mb-2 font-semibold text-xl">{title}</h3>
        <p className="flex-grow text-secondary-foreground">{description}</p>
      </div>
    </div>
  );
};
